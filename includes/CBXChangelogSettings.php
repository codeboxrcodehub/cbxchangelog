...
if ( ! class_exists( 'CBXChangelogSettings' ) ):
	...
	class CBXChangelogSettings {
		...
		function callback_heading( $args ) {
			$plus_svg  = cbxchangelog_load_svg( 'icon_plus' );
			$minus_svg = cbxchangelog_load_svg( 'icon_minus' );
			...
			$html = '<h3 class="setting_heading"><span class="setting_heading_title">' . esc_html( $args['name'] ) . '</span><a title="' . esc_attr__( 'Click to show hide',
					'cbxchangelog' ) . '" class="setting_heading_toggle button outline primary icon-only icon-inline" href="#"><i class="cbx-icon cbx-icon-img setting_heading_toggle_plus">' . $plus_svg . '</i><i class="cbx-icon cbx-icon-img setting_heading_toggle_minus">' . $minus_svg . '</i></a></h3>';
		...
		function callback_multicheck( $args, $value = null ) {
			...
			$html = '<p class="grouped gapless grouped_buttons checkbox_fields_check_actions"><a href="#" class="button primary checkbox_fields_check_action_call">' . esc_html__( 'Check All', 'cbxchangelog' ) . '</a><a href="#" class="button outline checkbox_fields_check_action_ucall">' . esc_html__( 'Uncheck All', 'cbxchangelog' ) . '</a></p>';
		...
		function callback_select( $args ) {
			...
			if ( $args['placeholder'] == '' ) {
				$args['placeholder'] = esc_html__( 'Please Select', 'cbxchangelog' );
			}
